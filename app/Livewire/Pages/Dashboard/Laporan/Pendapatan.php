...
namespace App\Livewire\Pages\Dashboard\Laporan;
...
class Pendapatan extends Component
{
    ...
    public function laporanHarian()
    {
        ...
        $pendapatans = PendapatanModel::whereDate('created_at', $date)
    ...
    public function laporanBulanan()
    {
        ...
        for ($date = $startDate->clone(); $date <= $endDate; $date->addDay()) {
            ...
            $dayPendapatan += PendapatanModel::whereDate('created_at', $date->toDateString())
                ->whereNotIn('sumber', ['Penjualan Barang', 'Pembelian Barang'])
                ->sum('jumlah');

            $weeks[$weekKey] += $dayPendapatan;
        }
    ...
    public function laporanTahunan()
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $total = Penjualan::whereBetween('created_at', [$startDate, $endDate])->sum('total');
            $total += PendapatanModel::whereBetween('created_at', [$startDate, $endDate])
                ->whereNotIn('sumber', ['Penjualan Barang', 'Pembelian Barang'])
                ->sum('jumlah');
    ...
    {
        $this->totalPendapatan = collect($this->laporan)->sum('jumlah');
    }
    ...
    {
        return view('livewire.pages.dashboard.laporan.pendapatan', [
