<div>
    <div class="p-8">
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-gray-900">Laporan Laba Rugi</h2>
            <p class="text-gray-600 mt-2">Analisis keuangan berdasarkan periode yang dipilih</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
            <div class="flex flex-col md:flex-row gap-4 items-end justify-between">
                ...
                <!-- Tombol Cetak -->
                <div>
                    <a href="{{ route('laporan.laba-rugi.cetak', ['type' => $filterType, 'tanggal' => $tanggal, 'bulan' => $bulan, 'tahun' => $tahun, 'tahunFilter' => $tahunFilter]) }}"
                        target="_blank"
                ...
                <table class="w-full">
                        ...
                        <tr class="border-b-2 border-gray-200">
                            ...
                            @else
                                ...
                                <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">
                                    {{ $filterType === 'bulan' ? 'Minggu Ke' : 'Bulan' }}</th>
                                <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700">Pendapatan</th>
                                <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700">Pengeluaran</th>
                                <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700">Uang Bersih</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            @if ($filterType === 'hari')
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                    ...
                                    <td class="py-4 px-4 text-sm">
                                        <span
                                            class="px-3 py-1 rounded-full text-xs font-medium {{ $row['tipe'] === 'pendapatan' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            {{ $row['sumber'] }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-4 px-4 text-right">
                                        <span
                                            class="text-sm font-bold {{ $row['tipe'] === 'pendapatan' ? 'text-green-600' : 'text-red-600' }}">
                                            {{ $row['tipe'] === 'pendapatan' ? '+' : '-' }}Rp
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                    ...
                                    <td class="py-4 px-4 text-right">
                                        <span class="text-sm font-bold text-green-600">+Rp
                                            {{ $formatRupiah($row['pendapatan']) }}</span>
                                    </td>
                                    <td class="py-4 px-4 text-right">
                                        <span class="text-sm font-bold text-red-600">-Rp
                                            {{ $formatRupiah($row['pengeluaran']) }}</span>
                                    </td>
                                    <td class="py-4 px-4 text-right">
                                        <span
                                            class="text-sm font-bold {{ $row['bersih'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                            {{ $row['bersih'] >= 0 ? '+' : '-' }}Rp
                                            {{ $formatRupiah(abs($row['bersih'])) }}
                                        </span>
                                    </td>
                                </tr>
                            @endif
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-gray-200 bg-gray-50">
                            @if ($filterType === 'hari')
                                <td colspan="2" class="py-4 px-4 text-sm font-bold text-gray-900">Total Pendapatan
                                </td>
                                <td colspan="2"></td>
                                <td class="py-4 px-4 text-right">
                                    <span class="text-sm font-bold text-green-600">+Rp
                                        {{ $formatRupiah($totalPendapatan) }}</span>
                                </td>
                            @else
                                <td colspan="4" class="py-4 px-4 text-sm font-bold text-gray-900">Total Pendapatan
                                </td>
                                <td class="py-4 px-4 text-right">
                                    <span class="text-sm font-bold text-green-600">+Rp
                                        {{ $formatRupiah($totalPendapatan) }}</span>
                        ...
                        @if ($filterType === 'hari')
                            ...
                            <tr class="bg-gradient-to-r from-green-50 to-green-100 border-t-2 border-green-300">
                                <td colspan="2" class="py-4 px-4 text-sm font-bold text-gray-900">Uang Bersih</td>
                                <td colspan="2"></td>
                                <td class="py-4 px-4 text-right">
                                    <span
                                        class="text-sm font-bold {{ $totalPendapatan - $totalPengeluaran >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $totalPendapatan - $totalPengeluaran >= 0 ? '+' : '-' }}Rp
                                        {{ $formatRupiah(abs($totalPendapatan - $totalPengeluaran)) }}
                                    </span>
                                </td>
                            </tr>
                        @else
                            <tr class="bg-gray-50">
                                <td colspan="3" class="py-4 px-4 text-sm font-bold text-gray-900">Total Pengeluaran
                                </td>
                            ...
                            <tr class="bg-gradient-to-r from-green-50 to-green-100 border-t-2 border-green-300">
                                <td colspan="3" class="py-4 px-4 text-sm font-bold text-gray-900">Total Uang Bersih
                                </td>
                                <td colspan="3" class="py-4 px-4 text-right">
                                    <span
                                        class="text-sm font-bold {{ $totalPendapatan - $totalPengeluaran >= 0 ? 'text-green-600' : 'text-red-600' }}">
